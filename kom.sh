...
https://transfer.sh/0EdHGW/hhd800.com@STARS-734-Comp.mp4
...
echo "::group::Kompres"
[[ -n "$FILEPATH1" ]] && ffmpeg -loglevel warning -i $FILEPATH1 -c:v libx264 -preset medium -tune film -vf 'fps=fps=30, scale=-2:480' -crf 28 -c:a copy "$NAME1"-Comp.mp4
[[ -n "$FILEPATH2" ]] && ffmpeg -loglevel warning -i $FILEPATH2 -c:v libx264 -preset medium -tune film -vf 'fps=fps=30, scale=-2:480' -crf 28 -c:a copy "$NAME2"-Comp.mkv
echo "::endgroup::"
